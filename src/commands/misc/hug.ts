...
import { randomRange, replaceChars, setCooldown } from "../../utils/utils";
...
export default {
    ...
    execute({ client, channel, args, userstate }) {
        ...
        if (user.toLowerCase() === "iaraaron" || replaceChars(user).includes("aaron")) {
            const index = randomRange(0, aaronHugResponses.length - 1);
        ...
        if (user.toLowerCase() === `${process.env.BOT_USERNAME}`.toLowerCase()) {
            const index = randomRange(0, kelleebotHugResponses.length - 1);
